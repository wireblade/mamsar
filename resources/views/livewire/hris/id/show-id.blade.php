<div
    class="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex flex-col items-center justify-center p-10 relative overflow-hidden"
>
    <a
        href="{{ url()->previous() }}"
    ...
    {{-- Decorative circles --}}
    <div
        class="absolute -top-20 -left-20 w-72 h-72 rounded-full border border-white/5 pointer-events-none"
    ></div>
    <div
        class="absolute -top-10 -left-10 w-48 h-48 rounded-full border border-white/10 pointer-events-none"
    ></div>
    <div
        class="absolute -bottom-20 -right-20 w-96 h-96 rounded-full border border-white/5 pointer-events-none"
    ></div>
    <div
        class="absolute -bottom-10 -right-10 w-56 h-56 rounded-full border border-white/10 pointer-events-none"
    ></div>
    <div
        class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full border border-white/5 pointer-events-none"
    ></div>

    {{-- Title --}}
    <div style="text-align:center; margin-bottom:30px;">
        <h2
            style="color:#ffffff; font-size:22px; font-weight:700; letter-spacing:2px; margin:0; text-transform:uppercase;"
        >
            Employee ID Card
        </h2>
        <div
            style="width:60px; height:3px; background:#f0c040; margin:10px auto 0; border-radius:2px;"
        ></div>
    </div>
    ...
    <div class="print-area flex items-center justify-center gap-6">
        ...
        >
            @if ($company == 'Mamsar' || $company == null)
                <img
                    src="{{ asset('storage/id_template/mamsar_front.jpg') }}"
                    style="width: 100%; height: 100%; display: block;"
                />
            @else
                <img
                    src="{{ asset('storage/id_template/zeman_front.png') }}"
                    style="width: 100%; height: 100%; display: block;"
                />
            @endif
            ...
            <div
                class="absolute text-center {{ $this->companyColor() }}"
                style="top:272px; left:0; right:0;"
            >
                <div
                    style="font-size:12px; font-weight:700; padding:4px 8px; line-height:1.2; "
                >
                    {{ $employeeId }}
                </div>
                ...
                <div
                    style="font-size:13px; font-weight:700; padding:4px 8px; line-height:1.2; margin-top:-8px; "
                >
                    {{ $position }}
                </div>
            </div>
        ...
        <!-- BACK -->
        <div
            class="relative {{ $this->companyColor() }}"
            id="id-back"
            style="width: 272px; height: 430px; overflow: hidden;"
        >
            @if ($company == 'Mamsar' || $company == null)
                <img
                    src="{{ asset('storage/id_template/mamsar_back.jpg') }}"
                    style="width: 100%; height: 100%; display: block;"
                />
            @else
                <img
                    src="{{ asset('storage/id_template/zeman_back.jpg') }}"
                    style="width: 100%; height: 100%; display: block;"
                />
            @endif
            ...
            >
                ...
                <div
                    class="px-6 grid grid-cols-[115px_10px_1fr] text-[11px] w-full font-semibold"
                >
                    <div class="text-left">Date of Birth</div>
                    <div class="text-center">:</div>
                    <div class="text-left ml-1.5">
                        {{ \Carbon\Carbon::parse($dob)->format('M j, Y') }}
                    </div>
                    ...
                    <div class="text-center">:</div>
                    <div class="text-left ml-1.5">{{ $marital_status }}</div>
                </div>

                {{-- <div class="font-size:10px; float:center; line-height:1.5; margin-top:25px; color:#000000;"> --}}

                <div
                    class="text-[12px] mt-3 {{ $this->companyColor() }} font-medium leading-normal w-full"
                >
                    <div class="w-3/4 mx-auto text-center font-semibold">
                        {{ $address }}
                    </div>
                ...
                <div
                    class="text-[12px] mt-2 {{ $this->companyColor() }} text-black font-medium leading-normal w-full"
                >
                ...
                @if ($company == 'Mamsar')
                    <div
                        class="text-[11px] mt-2 text-black font-normal text-justify p-3 leading-normal w-full"
                    >
                        This card is the property of
                        <font class="font-extrabold"
                            >MAMSAR CONSTRUCTION AND INDUSTRIAL
                            CORPORATION</font
                        >
                        and must be surrendered upon termination of employement.
                    </div>
                @else
                    <div
    ...
    {{-- Download Buttons --}}
    <div
        style="display:flex; gap:12px; justify-content:center; margin-top:30px; flex-wrap:wrap;"
    >
        <button
            onclick="downloadCard('id-front', 'ID_Front_{{ $employeeId }}')"
        ...
        <button
            onclick="downloadCard('id-back', 'ID_Back_{{ $employeeId }}')"
            ...
            allEls.forEach(el => {
                ...
                props.forEach(prop => {
                    try {
                        ...
                        if (val && (val.includes('oklch') || val.includes('oklab') || val.includes(
                                'color('))) {
            ...
            try {
                const canvas = await html2canvas(element, {
                    ...
                    onclone: function(clonedDoc) {
                        // Hide resize handles
                        clonedDoc.querySelectorAll('.resize-handle').forEach(el => el.style.display =
                            'none');
